...
#Antrates a random number between 1 and the number of cities minus 1.
def Rand_City(size):
...
def Find_Fitness(Ant, cities):
    fitness = 0
    for i in range(len(Ant)-1):
        if cities[Ant[i]][Ant[i+1]] == 0:
            return sys.maxint
        else:
            fitness += cities[Ant[i]][Ant[i+1]]
    
    fitness += cities[Ant[0]][Ant[-1]]

    return fitness


def ProbabilityToMove(v, cities, pheromone):
    p = []
    for i in range(len(pheromone)):
        if cities[v[-1]][i] != 0 and Is_New(v, i):
            if len(p) != 0:
                p.append((i, p[-1][1]+pheromone[v[-1]][i]))
            else:
                p.append((i, pheromone[v[-1]][i]))
    
    probability = Rand_City(p[-1][1])
    i = 0
    while i < len(p) and probability >= p[i][1]:
        i += 1
...
def SetPopulation(number_of_population, n):
    Population = []
    for _ in range(number_of_population):
        Population.append([Rand_City(n)])
    
    return Population


def Sum(pheromone, new_pheromone, Evap):
    n = len(pheromone)
    for i in range(n):
        for j in range(n):
            Evap = Rand_City(101)
            pheromone[i][j] =  max(pheromone[i][j]*(Evap)/100, 1) + new_pheromone[i][j] 


def FindBest(Population, cities):
    best_fitness = sys.maxint
    path = []
    for i in range(len(Population)):
        fitness = Find_Fitness(Population[i], cities)
...
def AntColony(number_of_iterations, cities, number_of_population, k, Evap):
    n = len(cities)
    pheromone = []
    Clear(pheromone,n, 1)
    best_fitness = sys.maxint
    ...
    for _ in range(number_of_iterations):
        Population = SetPopulation(number_of_population, n)
        new_pheromone =[]
        Clear(new_pheromone,n, 0)
        for _ in range(n-1):
            for i in range(len(Population)):
                next = ProbabilityToMove(Population[i][:], cities, pheromone)
                new_pheromone[Population[i][-1]][next] += k/cities[Population[i][-1]][next]
                Population[i].append(next)
        fitness, path = FindBest(Population, cities)
        if best_fitness > fitness:
            best_fitness = fitness
            best_path = path
        Sum(pheromone, new_pheromone, Evap)
        print("best_fitness:")
        print(best_fitness)
        print(best_path)
...
number_of_population = len(cities)//2
k = 1000
Evap = 80
AntColony(number_of_iterations, cities, number_of_population, k, Evap)